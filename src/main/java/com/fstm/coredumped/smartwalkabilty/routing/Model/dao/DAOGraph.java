...
public class DAOGraph implements IDAOGraph {
    ...
    public Graph getTheGraph(GeoPoint source, GeoPoint target) {
        ...
        try
        {
            ...
            PreparedStatement preparedStatement = c.prepareStatement("select * FROM ways WHERE st_contains((\n" +
                    "SELECT ST_BUFFER (\n" +
                    "\tST_SetSRID(ST_Point(?, ?),?)\n" +
                    "\t, ?, ?\n" +
                    ")), the_geom);");
...
//            preparedStatement.setString(6, geom);

            ResultSet resultSet = preparedStatement.executeQuery();

            while (resultSet.next()){
                Vertex v = new Vertex();
                v.setDepart(new GeoPoint(resultSet.getDouble("x1"), resultSet.getDouble("y1")));
                v.setArrive(new GeoPoint(resultSet.getDouble("x2"), resultSet.getDouble("y2")));
